...
// Local development:
// VITE_API_BASE_URL not set → http://localhost:8080/api
//
// Render production:
// VITE_API_BASE_URL=https://keystone-backend-id49.onrender.com
// → https://keystone-backend-id49.onrender.com/api
...
function LoginPage({
  ...
  onLogin: (t: string, m: LoginResult) => void;
}) {
  ...
  return (
    <div className="login-page">
      <div className="login-card">
        ...
        <form onSubmit={submit}>
          <div className="form-group">
            ...
            <input
              ...
              placeholder="manager@keystone.example.com"
              required
            />
          </div>
        ...
        <p
          className="text-muted text-sm mt-16"
...
function ManagerDispatcherView({
  token,
  me,
  onUnauth,
}: {
...
function WorkOrderBoard({
  ...
  const load = useCallback(async () => {
    try {
      ...
      const p: Page<WorkOrder> = await api(
        "/work-orders?page=0&size=200",
        token
      );
      ...
      if (
        ...
        setErr("Failed to load work orders.");
      }
    } finally {
      setLoading(false);
    }
  ...
  async function assign(
    ...
    try {
      ...
      const updated: WorkOrder = await api(
        `/work-orders/${orderId}/assign?assigneeId=${techId}`,
        token,
        {
          method: "PATCH",
        }
  ...
  async function transition(
    ...
    try {
      ...
      const updated: WorkOrder = await api(
        `/work-orders/${orderId}/status?status=${status}`,
        token,
        {
          method: "PATCH",
        }
...
function WorkOrderList({
  ...
  const [page, setPage] =
    useState<Page<WorkOrder> | null>(null);
...
function OverdueView({
    ...
    api<WorkOrder[]>(
      ...
      .catch((e: unknown) => {
        ...
        if (
          ...
          setErr(
            "Failed to load overdue work orders."
          );
        }
      })
  ...
  return (
    <div>
      ...
      {orders.length === 0 ? (
        <div className="empty-state">
          ✅ No overdue work orders — SLA is on track.
        </div>
      ) : (
        <div className="card table-wrap">
          <div className="alert alert-warn mb-16">
            {orders.length} work order(s) have
            breached SLA.
          </div>
            ...
            <tbody>
              {orders.map((o) => (
                <tr key={o.id}>
                  ...
                  <td className="sla-breached text-sm">
                    {fmt(o.slaDueDate)}
                  </td>
                </tr>
              ))}
    ...
    <div>
      ...
      <div className="stat-grid">
        ...
        <div className="card stat-card">
          ...
          <div
            className={`stat-value ${
              data.slaCompliancePercentage >= 90
                ? "success"
                : data.slaCompliancePercentage >= 70
                ? "warn"
                : "danger"
            }`}
          >
            {data.slaCompliancePercentage.toFixed(
              1
            )}
            %
          </div>
...
const TECH_TRANSITIONS: Partial<
...
function TechTimeLog({
  ...
  async function submit(
    ...
    if (!orderId || !minutes) {
      setMsg({
        text: "Select a job and enter minutes.",
        type: "error",
      });
    ...
    try {
      ...
      const qs = new URLSearchParams({
        technicianId: String(me.userId),
        minutes,
        ...(note ? { note } : {}),
      });

      await api(
        `/work-orders/${orderId}/time-logs?${qs}`,
        token,
        {
          method: "POST",
        }
...
// ─────────────────────────────────────────────
// Technician Parts Log
// ─────────────────────────────────────────────

function TechPartsLog({
  orders,
  parts,
  token,
  me: _me,
    ...
    <main>
      ...
      {loading ? (
        ...
        <>
          ...
          {orders.length === 0 ? (
            <div className="empty-state">
              No work orders yet. Contact your
              service provider to raise a request.
            </div>
          ) : (
            <div className="grid-2">
              {orders.map((o) => (
